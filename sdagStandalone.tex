    ...
    \item For any class that has Structured Dagger in it you must insert two
      calls:
       \begin{itemize}
         ...
         \item Call the \code{\_\_sdag\_pup()} in the \code{pup} method
       \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \lstinputlisting[basicstyle=\tiny]{code/jacobi3dELLWOReadonly.ci}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Jacobi: .ci file}
  \lstinputlisting[basicstyle=\tiny,linerange={22-49}]{code/jacobi3dSYNCWOReadonly.ci}
\end{frame}
